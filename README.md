...
- **검증틀**: `backtest.py`(슬리피지·ADV 반영), `walkforward.py`(Purge&Embargo 누수방지),
  ...
  팩터 변형 A/B 지원: `--mom-days/--mom-skip`(모멘텀 룩백), `--w-high52`(52주 신고가),
  `--vol-target`(변동성 타기팅). walkforward 도 같은 인자로 OOS 맞대결 가능.
- 핵심 교훈: 낙폭은 손절이 아니라 **국면필터**가 잡는다. 타이트 손절·트레일링은 휩쏘로 수익 악화.
- **해외 검증 팩터 테스트 결과(2026-06, 워크포워드 OOS 13폴드)**: 학계 표준 12-1 모멘텀(252/21),
  120일 룩백, 변동성 타기팅(Barroso & Santa-Clara) 모두 현행(60/5)에 패배. 52주 신고가
  (George & Hwang)는 전구간 백테스트에선 샤프 개선(0.88→0.92)이나 **WF OOS에선 역전 패배**
  (0.87→0.81) = 인샘플 함정. → 라이브 기본값 유지, 팩터는 opt-in 연구용으로만 보존.
- **검증 후 기각된 외부 전략(2026-06)** — 솔깃한 평판 ≠ 우리 데이터의 OOS 우위:
  - *유니버스 축소*(한국시장 논문 arXiv:1211.6517 "초대형주가 모멘텀 방해"):
    `--skip-top`(backtest/walkforward)으로 검증 결과 정반대 — 유동성 top10/20 제외 시
    WF OOS 샤프 0.87→0.63→0.48 단조 악화. 우리 blend 전략엔 초대형주가 기여.
  - *변동성 돌파(래리 윌리엄스 K-돌파, 국내 자동매매 봇 사실상 표준)*:
    `python -m src.research_vb` — 10년 OHLC + 실비용(왕복 25bps+슬리피지 20bps) 검증
    결과 전 구간 참패(거래당 평균 −0.13~−0.35%, K 0.3/0.5/0.7 모두 · ETF/개별주 모두).
    돌파 엣지 자체가 거래비용보다 작아 구조적으로 비용을 못 넘는다.
...
- **RVOL(상대거래량)** = 당일 누적거래량 ÷ (20일 평균 × 장 경과비율). 해외 검증
  (Zarattini·Barbon·Aziz 2024, 미국 7천종목 2016~23: 'Stocks in Play'=RVOL 상위로
  제한할 때만 단타 수익 유의미)을 근거로 **피처로 기록**하며, `min_rvol`/`rank_by: rvol`
  로 선정 규칙에도 쓸 수 있다(기본 OFF — 국내 데이터로 확인 후 켜는 것을 권장).
- **VWAP 괴리(vwap_dev)** = 현재가/당일VWAP − 1 (VWAP=누적거래대금÷누적거래량,
  스캔 응답만으로 산출 — 추가 API 호출 없음). 해외 ORB 봇 표준 룰 'VWAP 위에서만
  롱'의 피처화. 항상 기록되며 `require_above_vwap: true`로 필터로도 켤 수 있다
  (기본 OFF — RVOL과 같은 점진 검증 원칙).

- **데이터 수집이 forward-only**(과거 백필 불가)인 호가·분봉은 **매일 장중 봇 + 마감 후 분봉수집**을
  빠짐없이 돌려야 쌓입니다.
- **지수 국면 필터**: 지수 급락 중이면 신규 매수 자동 중단(Track 1 방어를 이식).
  KODEX200 + KODEX코스닥150 **둘 중 최악값** 기준 — 스캔 유니버스가 전체시장(코스닥
  다수)인데 코스피200만 보면 코스닥 단독 급락을 놓침(10년 일봉 분석: 268일=연평균
  27일, 그날 코스닥 평균 −2.26%). 학습 피처(index_chg)는 KODEX200 단일 의미 유지.
